...
class phpspider
{
    ...
    public function __construct($configs = array())
    {
        ...
        self::$collect_urls = isset($configs['collect_urls']) ? $configs['collect_urls'] : self::$collect_urls;
        self::$collect_urls_num = isset($configs['collect_urls_num']) ? $configs['collect_urls_num'] : self::$collect_urls_num;
        ...
        if ($status) {
            ...
            if (!empty(self::$configs['export'])) {
                ...
                if (self::$export_type == 'csv') {
                    ...
                    // if (empty($row)) {
                    //     $result = db::insert(self::$export_table, [
                    //         self::$configs['export']['unique_column'] => $url,
                    //         self::$configs['export']['created_at_column'] => date("Y-m-d H:i:s", time()),
                    //         self::$configs['export']['spider_original_url_column'] => $url,
                    //     ]);
                    //     if (empty($result)) {
                    //         var_dump($url);
                    //     }
    ...
    public function start()
    {
        ...
        if (self::$multiserver || self::$save_running_state || self::$tasknum > 1) {
            ...
            if (!queue::init()) {
                if (self::$multiserver) {
                    exit('Multiserver needs Redis support, ' . queue::$error);
                    exit;
                }

                if (self::$tasknum > 1) {
                    exit('Multitasking needs Redis support, ' . queue::$error);
                    exit;
                }

                if (self::$save_running_state) {
                    exit('Spider kept running state needs Redis support, ' . queue::$error);
                    exit;
                }
            }
        }
        ...
        if (!empty(self::$configs['export'])) {
            ...
            if (self::$export_type == 'csv') {
                ...
                // $count = db::get_one("SELECT count(*) as `collect_urls_num` FROM `" . self::$export_table . "`");
                // var_dump($count['collect_urls_num']);
                // db::update(self::$export_table, $fields, ["`" . self::$configs['export']['unique_column'] . "` = '" . $page['url'] . "'"]);
        ...
        foreach (self::$configs['scan_urls'] as $url) {
            ...
            if (!$this->is_scan_page($url)) {
                exit("Domain of scan_urls (\"{$url}\") does not match the domains of the domain name");
                exit;
            }
        }
    ...
    public function fork_one_task($taskid)
    {
        ...
        elseif (0 === $pid) {
            ...
            exit("Fork children task({$taskid}) fail...");
            exit;
        }
    }
    ...
    public function collect_page()
    {
        ...
        if (isset(self::$configs['log_type']) and strstr(self::$configs['log_type'], 'info')) {
            // var_dump(self::$configs['log_type']);
        ...
        if ($link['url_type'] == 'scan_page') {
            ...
            self::$collected_content_urls_num++;
        ...
        if ($http_code != 200) {
            ...
            if ($http_code == 301 || $http_code == 302) {
                ...
                if (!empty(self::$configs['max_try']) and in_array($http_code, ['407', '0', '502', '503', '429'])) {
                    ...
                    exit("Failed to download page {$url}, retry({$link['try_num']})");
                } else {
                    exit("Failed to download page {$url}");
                    self::$collect_fail++;
                }
                exit("HTTP CODE: {$http_code}");
                return false;
            }
        }
    ...
    {
        // var_dump(__FUNCTION__);
        $fields = empty($fields) ? $this->get_fields(self::$configs['fields'], $html, $url, $page) : $fields;
        ...
        if (!empty($fields)) {
            ...
            if (isset($fields) && is_array($fields)) {
                ...
                // 如果设置了导出选项
                // var_dump($fields);
                if (isset(self::$configs['export']['unique_column']))
                    $fields[self::$configs['export']['unique_column']] = $page['url'];
                if (isset(self::$configs['export']['created_at_column']))
                    $fields[self::$configs['export']['created_at_column']] = date("Y-m-d H:i:s", time());
                if (isset(self::$configs['export']['spider_original_url_column']))
    ...
    public function get_fields($confs, $html, $url, $page)
    {
        ...
        foreach ($confs as $conf) {
            ...
            if (empty($conf['name'])) {
                exit("The field name is null, please check your \"fields\" and add the name of the field\n");
                exit;
            }
        ...
        if (!empty(self::$configs['export'])) {
            if (self::$export_type == 'csv') {
                if (empty(self::$export_file)) {
                    exit('Export data into CSV files need to Set the file path.');
                    exit;
                }
            } elseif (self::$export_type == 'sql') {
                if (empty(self::$export_file)) {
                    exit('Export data into SQL files need to Set the file path.');
                    exit;
                }
            } elseif (self::$export_type == 'db') {
                if (!function_exists('mysqli_connect')) {
                    exit('Export data to a database need Mysql support, unable to load mysqli extension.');
                    exit;
                }

                if (empty(self::$db_config)) {
                    exit('Export data to a database need Mysql support, you have not set a config array for connect.');
                    exit;
                }
                ...
                if (mysqli_connect_errno()) {
                    exit('Export data to a database need Mysql support, ' . mysqli_connect_error());
                    exit('Export data to a database need Mysql support, ' . mysqli_connect_error());
                }
                ...
                if (!db::table_exists(self::$export_table)) {
                    exit('Table ' . self::$export_table . ' does not exist');
                    exit;
                }
            }
        }
    ...
    public function get_fields_xpath($html, $selector, $fieldname)
    {
        ...
        if (selector::$error) {
            exit("Field(\"{$fieldname}\") " . selector::$error . "\n");
        }
        return $result;
    }
