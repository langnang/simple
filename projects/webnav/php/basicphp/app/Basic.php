...
class Basic
{
    ...
    {
        if ($http_method === 'ANY')
            $http_method = $_SERVER['REQUEST_METHOD']; // Any HTTP Method
    ...
    public static function view($view, $data = NULL)
    {
        ...
        if (file_exists($file) && is_readable($file) && pathinfo($file)['extension'] === 'php')
            require_once $file; // Render page view
    }
    ...
    public static function apiCall($url, $http_method = 'GET', $data = NULL, $user_token = NULL)
    {
        if (substr(strtolower(trim($url)), 0, 16) !== 'http://localhost' && substr(strtolower(trim($url)), 0, 8) !== 'https://')
            self::apiResponse(400, 'API URL should start with "https://".'); // Require HTTPS API URL
    ...
    public static function apiResponse($code, $data = NULL, $content_type = 'text/plain')
    {
        ...
        if ($code > 199 && $code < 300)
            $message = 'OK'; // OK response
        if ($code < 200 || $code > 299)
            $message = $data; // If no data, $data = $message
    ...
    public static function setJsonRpc($controller = 'Controller')
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] !== 'GET' && $_SERVER['REQUEST_METHOD'] !== 'POST')
            exit(json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32601, 'message' => 'Only GET and POST methods allowed.'], 'id' => NULL])); // Only GET and POST
        if ($_SERVER['HTTP_CONTENT_TYPE'] !== 'application/json')
            exit(json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32700, 'message' => "Request content type should be 'application/json'."], 'id' => NULL])); // Accept only JSON request content type
        if (!$body)
            exit(json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32700, 'message' => 'Request should have a request body.'], 'id' => NULL])); // Require request body
        if ($body && !$array)
            exit(json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32700, 'message' => 'Provide request body data in valid JSON format.'], 'id' => NULL])); // Require valid JSON
        ...
        if (is_array($array) && !empty($array[0])) {
            ...
            foreach ($array as $json) {
                ...
                if (!isset($json['jsonrpc']) || $json['jsonrpc'] !== '2.0')
                    $res .= json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32600, 'message' => "JSON-RPC 'version' member should be set, and assigned a value of '2.0'."], 'id' => $json_id]) . ',';
                if (!isset($json['method']) || !strstr($json['method'], '.'))
                    $res .= json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32600, 'message' => "JSON-RPC 'method' member should be set with the format 'class.method'."], 'id' => $json_id]) . ',';
        ...
        if (!isset($array['jsonrpc']) || $array['jsonrpc'] !== '2.0')
            exit(json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32600, 'message' => "JSON-RPC 'version' member should be set, and assigned a value of '2.0'."], 'id' => $array_id]));
        if (!isset($array['method']) || !strstr($array['method'], '.'))
            exit(json_encode(['jsonrpc' => '2.0', 'error' => ['code' => -32600, 'message' => "JSON-RPC 'method' member should be set with the format 'class.method'."], 'id' => $array_id]));
